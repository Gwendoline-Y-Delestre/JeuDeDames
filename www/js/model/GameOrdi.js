...
export default class Game {
  ...
  /**
   * Renvoie la case SVG à partir de ses coordonnées
   * @param {Damier} damierObj 
   */
  newTurnOrdi(damierObj) {
    console.log("Nouveau tour Ordi");
    this.currentPlayer = "black";
    console.log(this.blackPions);
    const blackPions = this.blackPions.slice();
    let foundMove = false;
    // Continue à chercher un mouvement jusqu'à en trouver un
    while (!foundMove && blackPions.length > 0) {
      const randomIndex = Math.floor(Math.random() * blackPions.length);
      const selectedPion = blackPions[randomIndex];
      // Obtenez les coordonnées du pion sélectionné
      const x = Math.floor(parseFloat(selectedPion.getAttribute("cy")) / this.tailleCase);
      const y = Math.floor(parseFloat(selectedPion.getAttribute("cx")) / this.tailleCase);
      let pion = damierObj.getCase(x, y).getPion();
      // Obtenez les choix de mouvement pour le pion sélectionné
      const moveChoices = pion.moveChoice(damierObj);
      if (moveChoices.length > 0) {
        foundMove = true; // Sort de la boucle si un mouvement valide est trouvé
        const randomMoveIndex = Math.floor(Math.random() * moveChoices.length);
        const selectedMove = moveChoices[randomMoveIndex];
        const newX = selectedMove[0];
        const newY = selectedMove[1];
        ...
        let newDamierObj = pion.move(damierObj, newX, newY, moveResult[2]);
        // MAJ de this.blackPions
        for (let i = 0; i < this.blackPions.length; i++) {
          if (this.blackPions[i] === selectedPion) {
            this.blackPions.splice(i, 1);
          }
        }
        this.blackPions.push(this.getPionSVG(newX, newY));
        // Si le pion a mangé un adversaire, essayez de manger à nouveau
        if (moveResult[1] !== null) {
          if (pion instanceof Pion) {
            this.moveAgainOrdi(damierObj, pion, x, y, moveResult[1]);
          }
          else {
            this.moveAgainOrdi(newDamierObj, pion, x, y, moveResult[1]);
          }
        }
        else {
          console.log(x+","+y)
          console.log(newX+","+newY)
          console.log(damierObj.damier.length - 1)
          console.log(this.getPionSVG(pion.getCoordonnee().getX(), pion.getCoordonnee().getY()))
          if (newX == damierObj.damier.length - 1 && pion instanceof Pion) {
        ...
        // Nouveau tour pour le joueur humain
        this.newTurn(damierObj);
      }
      else {
        // Aucun choix de mouvement disponible pour le pion sélectionné, passe au prochain tour
        blackPions.splice(randomIndex, 1); // Retire le pion de la liste
      }
    }

    // Si la boucle se termine et qu'aucun mouvement n'a été trouvé, annonce la défaite
    if (!foundMove) {
      //TODO : retour au menu victoire du joueur
      // alert("L'ordinateur ne peut pas effectuer de mouvement. Il perd !");
  ...
  /**
   * Permet à un pion de manger un pion adverse si possible
   * @param {Damier} damierObj 
   * @param {Piece} pion 
   * @param {int} x 
   * @param {int} y 
   * @param {*} result 
   */
  moveAgainOrdi(damierObj, pion, x, y, result) {
    let eatChoices = [];
    if (pion instanceof Pion) {
    ...
    if (eatChoices.length > 0) {
      const randomMoveIndex = Math.floor(Math.random() * eatChoices.length);
      const selectedMove = eatChoices[randomMoveIndex];
      const newX = selectedMove[0];
      const newY = selectedMove[1];
      // Effectuez le déplacement graphique et dans le modèle
      let moveResult;
      ...
      // MAJ de this.blackPions
      for (let i = 0; i < this.blackPions.length; i++) {
        if (this.blackPions[i] === this.getPionSVG(x, y)) {
          this.blackPions.splice(i, 1);
        }
      }
      this.blackPions.push(this.getPionSVG(newX, newY));
  ...
  movePionSVG(pion, x, y) {
    ...
    if (Math.abs(x - pion.getCoordonnee().getX()) === 2 && Math.abs(y - pion.getCoordonnee().getY()) === 2) {
      ...
      console.log("PION MANGE PAR PION:")
      console.log(eatenCircle)
      if (eatenCircle !== null) {
 ...
 */
  moveDameSVG(dame, x, y) {
    ...
    if (diagonalCells.length !== 0) {
      ...
      console.log("PION MANGE PAR DAME:")
      console.log(eatenCircle)
    }
    else {
      eatenCircle = null;
